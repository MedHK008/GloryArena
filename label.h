...
texte *initialiser_texte(gint w, gint h, gchar* lb, guint alg, gchar* police, gint size, gchar* styl, gchar* epaisseur, gchar* couleur, gchar* background, gchar* underline)
{
    ...
    if (styl)
     ...
     if(background)
         strcpy(tex->couleur_fond,background);
    else strcpy(tex->couleur_fond,"");
...
GtkWidget *style_texte(GtkWidget *element, texte *text, gint type)
{
    ...
    switch (type)
    {
        case 1: // Cas o� l'�l�ment est un label (GTK_LABEL)
            ...
            if(strcmp(text->couleur_fond,""))
                            sUtf8 = g_strdup_printf("<span  background=\"%s\" font_desc=\"%d\" style=\"%s\" foreground=\"%s\"  font_family=\"%s\" underline=\"%s\">%s</span> ",text->couleur_fond, text->taille_text,text->style,text->couleur_text,text->police,text->underline,text->text);
            else
                sUtf8 = g_strdup_printf("<span  font_desc=\"%d\" style=\"%s\" foreground=\"%s\" font_family=\"%s\" underline=\"%s\">%s</span> ",text->taille_text,text->style,text->couleur_text,text->police,text->underline,text->text);
...
Etiquette* add_label(GtkWidget *pere,texte *text,gboolean selectable,gboolean wrap,gint x,gint y)
{
    ...
    return ((Etiquette*)label);
}

// Fonction pour libérer la mémoire allouée à une structure texte
void free_texte(texte *tex) {
    if (tex)
        // Pas besoin de libérer les champs car ils sont statiques dans ce cas
        g_free(tex);
}

// Fonction pour libérer la mémoire allouée à une structure Etiquette
void free_etiquette(Etiquette *label) {
    if (label) {
        // Libérer le texte associé à l'étiquette
        if (label->text)
            free_texte(label->text);
        // Libérer la structure Etiquette elle-même
        g_free(label);
    }
}
